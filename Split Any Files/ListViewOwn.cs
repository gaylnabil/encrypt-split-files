...
namespace Split_Any_Files
{
    ...
    public  partial class ListViewOwn : ListView
    {
        ...
        private void cmbx_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            if (sender.GetType() == typeof(ComboBoxOwn))
            {
                ComboBoxOwn excmbx = (ComboBoxOwn)sender;
                object item = excmbx.SelectedItem;
                //Is this an combobox item with one image?
                if (item.GetType() == typeof(ComboBoxOwn.EXImageItem))
                {
                    ComboBoxOwn.EXImageItem imgitem = (ComboBoxOwn.EXImageItem)item;
                ...
                else if (item.GetType() == typeof(ComboBoxOwn.EXMultipleImagesItem))
                {
                    ComboBoxOwn.EXMultipleImagesItem imgitem = (ComboBoxOwn.EXMultipleImagesItem)item;
                    if (_col == 0)
                    {
                        if (_clickeditem.GetType() == typeof(EXImageListViewItem))
                        {
    ...
    public class EXListViewItem : ListViewItem
    {
        ...
        public EXListViewItem(string text,int indexImage) : base(text,indexImage)
        {
        }
